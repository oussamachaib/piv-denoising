...
std = .05;
%std = ss(s); % as a fraction of the maximum intensity
var = std^2;
...
% real vs. noisy displacement field RMS error
RMSEn(s) = sqrt(mean(mean((Un-U).^2)))

% real vs. wavelet denoised displacement field RMS error
RMSEd(s) = sqrt(mean(mean((Ud-U).^2)))

% real vs. median denoised displacement field RMS error
RMSEd1(s) = sqrt(mean(mean((Ud1-U).^2)))

% real vs. mean denoised displacement field RMS error
RMSEd2(s) = sqrt(mean(mean((Ud2-U).^2)))

% real vs. wiener denoised displacement field RMS error
RMSEd3(s) = sqrt(mean(mean((Ud3-U).^2)))
...
%% RMSE random samples
figure()
plot(RMSEn,'.','MarkerSize',20)
hold on;
plot(RMSEd,'.','MarkerSize',20)
hold on;
plot(RMSEd1,'.','MarkerSize',20)
hold on;
plot(RMSEd2,'.','MarkerSize',20)
hold on;
plot(RMSEd3,'.','MarkerSize',20)
hold off;
%leg1=legend('Noise','Wavelet','Median','Mean','Wiener');
%set(leg1,'Interpreter','latex');
xlabel('Random sample number [-]','Interpreter','latex')
ylabel('Root mean square error [px]','Interpreter','latex')
title('$\sigma = 0.10$','Interpreter','latex');
...
xlim([0, 51])
%ylim([0, 1])
%saveas(gcf,'figures/denoising/random_new_sigma2.jpg')
